...
70 : -- -- -- -- -- -- -- --
    ...
    output = subprocess.check_output(['i2cdetect', '-y', '1']).decode('utf-8') #'i2cdetect', '-y', '1'
    ...
    available_devices_markup = '<span font="20">Available device(s) at addresses: '
    devices_found = False
    # Skip the first line (column headers)
    for line in lines[1:]:
        # Split the line into elements
        elements = line.split()
        row_address = int(elements[0].replace(':', ''), 16)  # Convert row address to integer from hex
        for i, element in enumerate(elements[1:]):  # Skip row address and colon
            if element != "--":
                devices_found = True
                # Calculate the device address correctly
                device_address = (row_address << 4) | i
                available_devices_markup += f"0x{device_address:02x}, "


    if not devices_found:
        available_devices_markup = '<span font="20">No devices found</span>'
    else:
        available_devices_markup = available_devices_markup.rstrip(', ') + '</span>'
    available_devices.set_markup(available_devices_markup)
